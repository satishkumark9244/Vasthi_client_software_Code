...
def getConvertedData(val, dtype):
    ...
    if dtype == "INTEGER":
        ...
        return struct.unpack('!d', val.decode('hex'))[0]
    # end V1.4
...
def generateInputString(selected_modbus_device_id, selected_modbus_register, selected_param_address, selected_length):
    ...
    final_input_string = interimn_input_string + CRC16_BIG_INDIAN(interimn_input_string.decode('hex'))
    return final_input_string.decode('hex')
    # End
...
def verifyOutCRC(out, outCRC):
    ...
    myCRC = CRC16_BIG_INDIAN(outStr.decode('hex'))
